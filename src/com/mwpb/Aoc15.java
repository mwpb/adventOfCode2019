...
public class Aoc15 {
    ...
    void addEdge(int x0, int y0, int x1, int y1) {
        List<String> oldForward = this.adjList.getOrDefault(this.getString(x0, y0), new LinkedList<>(List.of()));
        List<String> oldBackward = this.adjList.getOrDefault(this.getString(x1, y1), new LinkedList<>(List.of()));
    ...
    boolean tryMove(int x, int y, int direction) {
        this.intcode.input = new LinkedList<>(List.of(direction));
        ...
        while (searchPath.size() > 0) {
            ...
            if (!visited.contains(this.getString(x, y + 1))) {
                ...
                if (searchPath.size() > 0) {
                    ...
                    System.out.println(String.format("(%d, %d) , (%d, %d): %d", x, y, x2, y2, direction2));
                    this.intcode.input = new LinkedList<>(List.of(direction2));
