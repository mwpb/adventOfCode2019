...
class Aoc18_2 {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			for (int j = 0; j < line.length; j++) {
				if (Character.isLetter(line[j])
						|| Set.of('0', '1', '2', '3').contains(line[j])) {
					this.pointsLookup.put(line[j], new Point(j, i));
					if (Character.isLetter(line[j])
							&& Character.isLowerCase(line[j])) {
	...
	boolean inBounds(Point p) {
		return (p.x >= 0) && (p.y >= 0) && (p.x < this.maze[0].length)
				&& (p.y < this.maze.length);
	}
	...
	void bfs(Point p1) {
		...
		while (q.size() > 0) {
			...
			int x = current.x;
			int y = current.y;
			for (Point next : List.of(new Point(x - 1, y), new Point(x + 1, y),
				...
				if (!visited.contains(next) && this.inBounds(next)) {
					...
					if (nextChar == '.') {
						...
						distances.put(next, distance + 1);
							...
							|| Set.of('0', '1', '2', '3').contains(nextChar)) {
						ArrayList<Edge> tmpList = this.adjList.getOrDefault(
								currentChar, new ArrayList<Edge>());
	...
	Set<Edge> nextKeys(PointState ps) {
		if (this.nextKeysCache.containsKey(ps)) {
			...
			while (q.size() > 0) {
				Edge edge = q.poll();
				if (Character.isLowerCase(edge.end)
						&& !keysCollected.contains(edge.end) && c != edge.end) {
					nextKeys.add(edge);
				} else if (!keysCollected
						.contains(Character.toLowerCase(edge.end))) {
				} else {
		...
		if (this.stepsCache.containsKey(rs)) {
			...
			for (Character robot : rs.robots) {
				...
				for (Edge edge : this.nextKeys(ps)) {
					...
					int dist = edge.length + this.mainDistance(
							new RobotsState(newRobots, newKeys, nFind - 1));
					if (dist < best) {
						best = dist;
					}
				}
