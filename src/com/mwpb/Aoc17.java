...
public class Aoc17 {
	...
	Aoc17(String instructionsString) {
		...
		this.memory = Arrays.copyOf(this.intcode.memory,
				this.intcode.memory.length);
...
//        this.view = view;
	...
	boolean isIntersection(char[][] view, int x, int y) {
		return x != 0 && y != 0 && y != view.length - 1
				&& x != view[0].length - 1 && view[y][x] == '#'
	...
	boolean canMove(char direction, int x, int y) {
		if (direction == '^' && y > 0 && this.view[y - 1][x] == '#') {
			...
			return true;
		} else if (direction == '>' && x < this.view[0].length - 1
				&& this.view[y][x + 1] == '#') {
			this.robotX++;
			this.robotOrientation = direction;
			return true;
		} else if (direction == 'v' && y < this.view.length - 1
				&& this.view[y + 1][x] == '#') {
			this.robotY++;
			this.robotOrientation = direction;
			return true;
		...
		while (!finished) {
			if (this.canMove(this.robotOrientation, this.robotX, this.robotY)) {
				this.instructions.add("1");
					...
					this.getLeftOrientation(this.robotOrientation), this.robotX,
					this.robotY)) {
				this.instructions.add("L");
				this.instructions.add("1");
			} else if (this.canMove(
					this.getRightOrientation(this.robotOrientation),
	...
	List<Long> asciiChars(List<Character> f) {
		List<Long> ascii = new LinkedList<Long>();
		for (char c : f) {
			ascii.add((long) (int) c);
			ascii.add(44L);
		}
	...
	List<String> removeOccurences(List<String> fullList, List<String> subList,
			int maxToRemove) {
		boolean finished = false;
	...
	void getMovementRoutine() {
		...
		while (tempInstructions.size() > 0) {
			if (Collections.indexOfSubList(tempInstructions, this.A) == 0) {
				tempInstructions = this.removeOccurences(tempInstructions,
						this.A, 1);
				this.movementRoutine.add('A');
			} else if (Collections.indexOfSubList(tempInstructions,
					this.B) == 0) {
				tempInstructions = this.removeOccurences(tempInstructions,
						this.B, 1);
	...
	int dustVolume() {
		...
		List<Character> movementRoutine = new LinkedList<>(
		...
		this.intcode.input.add(110L);
//        this.intcode.input.add(121);
		this.intcode.input.add(10L);
